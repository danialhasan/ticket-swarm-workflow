...
description: Use after Danial has annotated a UI failure pattern and future agents should proactively find, classify, and route the same class of product-surface abstraction mistake without requiring Danial to annotate every repeated instance.
---
...
and the system should stop asking Danial to point at every repeat instance.
...
for, routed, and no longer depends on Danial manually marking every copy.

The skill must also prevent annotation downgrades: once Danial marks a visible
...
is fixed, hidden, proven inaccessible, or explicitly deferred by Danial.

## Use When

- Danial has corrected the same UI problem more than once.
...
- The next agent should inspect the app and propose fixes before asking Danial
  for more annotation.
...
## Common Rules From Current Dogfood
...
Ask Danial only for `new-design-question` or high-impact `nearby-problem`
...
dogfood user cannot encounter it or with Danial's explicit deferral.
...
- `blocker`: prevents dogfood/product flow until fixed or made truthful.
...
only when Danial explicitly approves it. Otherwise the disposition must be
...
- `$goal-loop-operator` when the rule changes the current dogfood goal ledger,
...
- Danial is not asked to annotate the same class of issue again unless a genuinely
